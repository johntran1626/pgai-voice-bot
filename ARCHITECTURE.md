...
A single Python process does three things at once. It runs a small FastAPI
server, exposes that server to the internet through a tunnel (ngrok or
cloudflared), and then asks Twilio to dial +1-805-439-8008. The TwiML we hand Twilio says
...
finished a thought, and its eagerness is tunable from `.env` because two AIs
talking to each other will otherwise stampede over one another — a model that would
...
a text model to think, ElevenLabs to speak. That shape was rejected here for
two reasons.
...
seconds of dead air per turn. Realistic pacing is the whole point of this
harness, and two seconds of silence after every sentence is precisely what
makes a voice agent sound broken. Speech-to-speech keeps it near 500ms
...
are precisely the signals needed to judge turn-taking quality and to make the
patient sound like a person rather than a script being read aloud.

The trade-off: less introspection. In a pipeline the exact text the model saw
before replying could be logged. Here the audio goes into a black box. This is
compensated for by asking OpenAI to transcribe the inbound audio *anyway*,
purely for the record, which yields both sides of the transcript without
putting transcription on the critical path.

### Twilio + a custom bridge instead of Vapi, Retell or Bland

A managed voice-agent platform would have collapsed this project into one
HTTP POST. This project started there — the first commit in this repo is a Vapi
...
managed platform that is a dashboard setting to fight; here it is a
...
build and has better-tuned endpointing out of the box. The control was judged
worth the extra day: "we couldn't examine the thing we were supposed to be
measuring" would have been the worse outcome.

### Running locally through ngrok instead of deploying

Twilio needs a public URL, and the reflex is to deploy to Render or Fly. This
project doesn't, because deployment adds a build step, a secrets store, and a
log console between the operator and the bug — and this is a tool run maybe
...
entirely and points at a deployed instance — that hook exists for exactly that.
...
anything the code dropped, and it puts each speaker on a separate stereo
...
"they overlapped for 800ms". It also arrives as `.mp3`, playable anywhere,
with no `ffmpeg` dependency.
...
- **`analyze.py` output is a draft, not a finding.** Language models
  confabulate plausible bugs. It produced 75 candidates; 13 survived being
  checked against the recordings by hand, and only those are reported.
